...
#这个文件的功能是批量测试raft系统，并在报错时将相应的日志存储保留起来；
#与dstest的区别是，记录的日志详细，展露更多细节；
#对于解决几千次才会出一次报错的bug非常有用；
...
def run_test(test: str, race: bool, timing: bool):
    ...
    ds_cmd = ["python3", "dslogs.py", "-c", "7", "-i", "TIMR"]

    #M
    # 使用临时文件来保存dslogs.py的输出
    _, path2 = tempfile.mkstemp()

    # 使用管道将go test的输出传递给dslogs.py
    with open(path2, 'w') as output_file:
        start = time.time()
        proc = subprocess.run(test_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, check=False)
        runtime = time.time() - start
        proc2 = subprocess.run(ds_cmd, input=proc.stdout, stdout=output_file, stderr=output_file, text=True, check=True)

    #M

    return test, path2, proc.returncode, runtime
